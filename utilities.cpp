...
void shrink(uint8_t *input, uint8_t *output, int ny, int nx, int nc, int factor) {
    ...
    int nxsml = nx/factor;
    uint32_t value = 0;
    //#pragma acc data copyin(input[0:nx*ny*nc]) copyin(ny) copyin(nx) copyin(nc) copy(output[0:ny*nx*nc/(factor*factor)]) copyin(value) copyin(factor) copyin(nxsml) copyin(nysml)
    //#pragma acc parallel loop 
...
void enlarge(uint8_t *input, uint8_t *output, int ny, int nx, int nc, int factor) {
    ...
    int nylrg = ny*factor;
    int nxlrg = nx*factor;
    #pragma acc data copyin(input[0:nx*ny/(factor*factor)]) copyin(nx) copyin(nc) copy(output[0:nylrg*nxlrg]) copyin(factor) copyin(nxlrg) copyin(nylrg)
    #pragma acc parallel loop 
