...
void shrink(uint8_t *input, uint8_t *output, int ny, int nx, int nc, int factor) {
    ...
    int nxsml = nx/factor;
    uint32_t value = 0;
    #pragma acc data copyin(input[0:nx*ny*nc]) copyin(ny) copyin(nx) copyin(nc) copy(output[0:ny*nx*nc/(factor*factor)]) create(value) copyin(factor) copyin(nxsml) copyin(nysml)
    #pragma acc parallel loop 
